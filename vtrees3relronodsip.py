...
class Branch(object):
    ...
    class Segment(object):
        def __init__(self, length, incr_unlo, incr_lo, branch, radius = 0,
                     inc='na', F='na', M='na', pos='na', end='na'):
            self.branch = branch
    ...
    def branchoff(self):
        ...
        while abs(self.inc + incmod) > self.char['maxinc']:
            incmod = (np.random.random() * self.char['incrange']*2) - self.char['incrange']
        self.children.append(self.__class__(np.pi/4, azmod, self.end, self)) #DEBUG ALERT
...
class Seed(Branch):
    ...
    def __init__(self, pos, char):
        self.az = np.random.random()*np.pi*2
        self.inc = 0
        self.order = -1
        self.pos = pos
        self.char = char
...
if __name__ == "__main__":
    ...
    def tspin(branch):
        if branch.children:
            ...
            branch.o += 0.01
            #branch.az = branch.get_az()
            #branch.inc = branch.get_inc()
            #branch.pos = branch.parent.get_point(branch.relativepos)
            #branch.resolve()
